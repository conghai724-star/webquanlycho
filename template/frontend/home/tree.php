...
$markets = $db->select("SELECT market_id AS id, market_name AS name, market_phone AS phone, '' AS address FROM markets WHERE market_status_code = 'active' ORDER BY market_name ASC");
...
<script>
window.BASE_URL = '<?php echo rtrim(parse_url(BASE_URL, PHP_URL_PATH) ?? "", "/") . "/"; ?>';
$(document).ready(function() {
    ...
    async function fetchStallTreeApi() {
        try {
            ...
            return {
                ...
                message: data ? data.message : 'Dữ liệu không hợp lệ.'
            };
        } catch (e) {
    ...
    async function loadTree() {
        ...
        if (res.success && res.data) {
            ...
            const errMsg = res.message || 'Mất kết nối máy chủ.';
            document.getElementById(CONFIG.SELECTOR.TREE_ROOT).innerHTML = 
                `<div style="color: red; padding: 10px;">Lỗi tải sơ đồ: ${errMsg}</div>`;
        }
    }
