        ...
        <form id="form-add-stall" action="<?php echo BASE_URL; ?>api/addStall" method="POST">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; margin-bottom: 20px;">
                    ...
                    <select id="stall_type_id" name="stall_type_id" class="form-control" required>
                        ...
                        <?php if (!empty($stallTypes)): ?>
                            <?php foreach ($stallTypes as $st): ?>
                                <option value="<?php echo ($st['stall_type_id'] ?? $st['status_id']); ?>" <?php echo ($data['stall_type_id'] ?? '') == ($st['stall_type_id'] ?? $st['status_id']) ? 'selected' : ''; ?>>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr 1.2fr; gap: 10px;">
                    <div class="form-group">
                        ...
                        <input type="number" step="0.01" min="0.01" id="x" name="x" class="form-control" placeholder="Dài" value="<?php echo htmlspecialchars($data['stall_map_coordinate_x'] ?? ''); ?>" required>
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="y" style="font-weight: 500;">Chiều rộng (m) <span style="color: var(--red)">*</span></label>
                        <input type="number" step="0.01" min="0.01" id="y" name="y" class="form-control" placeholder="Rộng" value="<?php echo htmlspecialchars($data['stall_map_coordinate_y'] ?? ''); ?>" required>
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="area_size" style="font-weight: 500;">Diện tích (m²)</label>
                        <input type="text" id="area_size" class="form-control" style="background-color: var(--bg-card); cursor: not-allowed;" placeholder="Tự tính" readonly value="<?php echo htmlspecialchars($data['stall_area_size'] ?? ''); ?>">
                    </div>
                </div>
            </div>
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; margin-bottom: 20px;">
                ...
                <div class="form-group">
                    ...
                    <input type="text" inputmode="numeric" id="base_price" name="base_price" class="form-control price-format" placeholder="Nhập giá cho thuê mỗi tháng" value="<?php echo !empty($data['stall_base_price']) ? number_format((float)$data['stall_base_price'], 0, ',', '.') : ''; ?>" required>
                </div>
                    ...
                    <select id="status" name="status" class="form-control">
                        <?php if (!empty($statuses)): ?>
                            <?php foreach ($statuses as $st): ?>
                                ...
                                <?php if ($st['status_code'] !== 'rented'): ?>
                                    <option value="<?php echo htmlspecialchars(($st['stall_type_id'] ?? $st['status_id'])); ?>" <?php echo ($data['stall_status_id'] ?? 3) == ($st['stall_type_id'] ?? $st['status_id']) ? 'selected' : ''; ?>>
...
$(document).ready(function() {
    ...
    // App.utils.handleFormSubmit('form-add-stall', '<?php echo BASE_URL; ?>admin/stalls');
    $('#form-add-stall').on('submit', function(e) {
        e.preventDefault();
        var form = this;
        var $form = $(this);
        ...
        App.utils.saveFormDraft('form-add-stall');
            ...
            success: function(data) {
                ...
                if (data.status === 200) {
                    Swal.fire({
                        ...
                        App.utils.clearFormDraft('form-add-stall');
