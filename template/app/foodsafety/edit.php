...
<div class="card">
    ...
    <div class="card-body" style="padding: 24px;">
        <form id="form-edit-cert" action="<?php echo BASE_URL; ?>api/editCertificate" method="POST" enctype="multipart/form-data">
            <?php csrf_field(); ?>
            <input type="hidden" name="id" value="<?php echo $certificate['attp_id']; ?>">
                    ...
                    <select id="trader_id" name="trader_id" class="form-control" required>
                        ...
                        <?php if (!empty($traders)): ?>
                            <?php foreach ($traders as $trader): ?>
                                <option value="<?php echo $trader['trader_id']; ?>" <?php echo (int)$certificate['attp_trader_id'] === (int)$trader['trader_id'] ? 'selected' : ''; ?>>
                                    <?php echo htmlspecialchars($trader['trader_fullname']); ?> (<?php echo htmlspecialchars($trader['trader_code']); ?><?php echo !empty($trader['trader_description']) ? ' - ' . htmlspecialchars($trader['trader_description']) : ''; ?>)
                                </option>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </select>
                    ...
                    <select id="doc_type_id" name="doc_type_id" class="form-control" required>
                        ...
                        <?php if (!empty($documentTypes)): ?>
                            <?php foreach ($documentTypes as $dt): ?>
                                <option value="<?php echo $dt['doc_type_id']; ?>" <?php echo $certificate['attp_doc_type_id'] == $dt['doc_type_id'] ? 'selected' : ''; ?>>
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; margin-bottom: 20px;">
                <div class="form-group">
                    ...
                    <input type="text" id="attp_name" name="attp_name" class="form-control" value="<?php echo htmlspecialchars($certificate['attp_name']); ?>" placeholder="Ví dụ: Giấy chứng nhận vệ sinh ATTP cửa hàng giò chả" required>
                </div>

                <div class="form-group">
                    <label class="form-label" for="doc_number" style="font-weight: 500;">Số quyết định / Số GCN <span style="color: var(--red)">*</span></label>
                    <input type="text" id="doc_number" name="doc_number" class="form-control" value="<?php echo htmlspecialchars($certificate['attp_doc_number']); ?>" placeholder="Ví dụ: 123/2026/GCNATTP-QLC" required>
                </div>
            </div>
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; margin-bottom: 20px;">
                <div class="form-group">
                    ...
                    <input type="text" id="issuer" name="issuer" class="form-control" value="<?php echo htmlspecialchars($certificate['attp_issuer']); ?>" placeholder="Ví dụ: Chi cục ATTP Hà Nội / UBND Quận">
                </div>
                ...
                <div class="form-group">
                    ...
                    <?php if (!empty($certificate['attp_file'])): ?>
                        <div style="margin-top: 10px;" id="edit-file-preview">
                            <?php 
                                $isPdf = strtolower(pathinfo($certificate['attp_file'], PATHINFO_EXTENSION)) === 'pdf';
                                $iconClass = $isPdf ? 'fa-solid fa-file-pdf' : 'fa-solid fa-image';
                            ?>
                            <a href="<?php echo BASE_URL . 'uploads/foodsafety/' . htmlspecialchars($certificate['attp_file']); ?>" target="_blank" style="color: var(--primary); text-decoration: underline; font-weight: 600; font-size: 13px;">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; margin-bottom: 20px;">
                <div class="form-group">
                    ...
                    <input type="date" id="issue_date" name="issue_date" class="form-control" value="<?php echo $certificate['attp_issue_date']; ?>" required>
                </div>

                <div class="form-group">
                    <label class="form-label" for="expiry_date" style="font-weight: 500;">Ngày hết hạn hiệu lực <span style="color: var(--red)">*</span></label>
                    <input type="date" id="expiry_date" name="expiry_date" class="form-control" value="<?php echo $certificate['attp_expiry_date']; ?>" required>
                </div>
            </div>
            ...
            <div class="form-group" style="margin-bottom: 24px;">
                ...
                <textarea id="attp_description" name="attp_description" class="form-control" rows="3" placeholder="Nhập thêm chi tiết về phạm vi được cấp phép, điều kiện kèm theo..."><?php echo htmlspecialchars($certificate['attp_description']); ?></textarea>
            </div>
